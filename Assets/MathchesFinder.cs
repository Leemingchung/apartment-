...
public class MathchesFinder : MonoBehaviour
{
    ...
    public void DFS(int x, int y)
    {
        ...
        for (int k = 0; k < 4; k++)
        {
            int xOther = x + dx[k];
            int YOther = y + dy[k];
            if (IsInsideBound(xOther, YOther) && !visited[xOther, YOther])
            {
    ...
    private static bool IsInsideBound(int xOther, int YOther)
    {
        return xOther >= 0 && xOther < 4 && YOther >= 0 && YOther < 6;
    }

    public void ResetVisited(int x, int y, Item currentItem)
    {
        for (int i = 0; i < 4; i++)
        {
            for (int j = 0; j < 6; j++)
        ...
        if (_listItem.Count >= 2)
        {
            ...
            ScoreManager.instance.UpdateScore(_listItem.Count * 2);
            _listItem.Clear();
    ...
    public IEnumerator WaitTime(float time, Action action)
    {
